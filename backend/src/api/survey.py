"""설문/온보딩 API - 카테고리 선택 → 논문 선택 → LLM 처리"""
...
def get_survey_papers(
    ...
    try:
        ...
        ### 수정사항: 현재 Paper 엔티티에 맞게 필드 수정 (authors 제거, primary_category relationship 접근)
        items = [
            PaperForSelection(
                paper_id=p.id,
                arxiv_id=p.arxiv_id,
                ...
                primary_category=p.primary_category.category.category_type if p.primary_category and p.primary_category.category else None,
...
def complete_survey(
    ...
    try:
        ...
        # 2. 저장된 키워드 가져오기
        profile = service.get_profile(payload.user_id)

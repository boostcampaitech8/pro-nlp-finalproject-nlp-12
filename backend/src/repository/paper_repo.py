...
class PaperRepository:
    ...
    def get_papers_as_documents(self, is_limited: bool = False, limit: Optional[int] = None) -> list[Document]:
        """
        DB에서 모든 논문 데이터 조회
        """
        query = self.db.query(Paper, Summary.summary_text).join(
        ...
        if is_limited:
            query.limit(limit).order_by(Paper.id)
